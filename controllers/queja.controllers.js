...
export const updateQueja = async (req, res) => {
  try {
    ...
    if (!queja) {
      ...
      return res.status(404).json({ message: `La queja con el ID: ${id} no existe 🤔` });
    }

    // Verifica que usuariosMegusta sea un array antes de actualizar
    if (!Array.isArray(usuariosMegusta)) {
      return res.status(400).json({ message: "usuariosMegusta debe ser un array" });
    }
    ...
    console.log(`Éxito al modificar el campo usuariosMegusta de la queja con el ID: ${queja.id} 😃`);
    res.json(queja);
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
